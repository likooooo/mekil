...
void transpose(const T* input, T* output, const std::array<int,2>& shape)
{
    ...
    MKL_REPEAT_CODE(T, omatcopy,
        ...
        rows, cols,
        1.0f,                                // alpha
        input, is_c_stly_memory_layout ? cols : rows,
        output, is_c_stly_memory_layout ? rows : cols
    );
}
template<class TFrom, class TTo, class Callback, bool is_c_stly_memory_layout = false>
void permuteND(const TFrom* input, TTo* output,
